...
bool LslidarC16Decoder::loadParameters() {
    ...
    angle3_disable_min = angle3_disable_min*M_PI/180.0;
    angle3_disable_max = angle3_disable_max*M_PI/180.0;
...
bool LslidarC16Decoder::createRosIO() {
    packet_sub = nh.subscribe<lslidar_c16_msgs::LslidarC16Packet>(
                "lslidar_packet", 10000000, &LslidarC16Decoder::packetCallback, this);
...
void LslidarC16Decoder::publishPointCloud() {
    ...
    //        lyh modified 18
    for(int i = 0;i<sweep_point_cloud.size();i++)
    {
        //printf("min = %f,max = %f\n",angle3_disable_min,angle3_disable_max);
        if(angle3_disable_min < angle3_disable_max)
        {
            if ((sweep_point_cloud[i].azimuth > angle3_disable_min) and (sweep_point_cloud[i].azimuth < angle3_disable_max))
            {
                continue;
            }
        }
        ...
        {
            if ((sweep_point_cloud[i].azimuth > angle3_disable_min) or (sweep_point_cloud[i].azimuth < angle3_disable_max))
            {
                continue;
            }
        }
        ...
        point.x = sweep_point_cloud[i].y;
        point.y = -1*sweep_point_cloud[i].x;
        point.z = sweep_point_cloud[i].z;
        point.time_offset_us = sweep_point_cloud[i].time;

//        std::cout<<std::fixed<<"point.time_offset_us = "<<sweep_point_cloud[i].time<<std::endl;
        point.intensity = sweep_point_cloud[i].intensity;
...
void LslidarC16Decoder::decodePacket(const RawPacket* packet) {
    ...
    for (size_t fir_idx = 1; fir_idx < FIRINGS_PER_PACKET; fir_idx+=4)
    {
        firings[fir_idx].firing_azimuth = firings[fir_idx+2].firing_azimuth;
    }

//    lyh 2019/12/31 modify for test azimuth interpolate
//    for (size_t fir_idx = 0; fir_idx < FIRINGS_PER_PACKET; fir_idx+=1)
//    {
//        std::cout<<"fir_idx = "<<fir_idx<<" firing_azimuth = "<<firings[fir_idx].firing_azimuth<<std::endl;
//    }


    // Fill in the distance and intensity for each firing.
    for (size_t blk_idx = 0; blk_idx < BLOCKS_PER_PACKET; blk_idx+=1) {
        ...
        for (size_t blk_fir_idx = 0; blk_fir_idx < FIRINGS_PER_BLOCK; ++blk_fir_idx){
            ...
            for (size_t scan_fir_idx = 0; scan_fir_idx < SCANS_PER_FIRING; ++scan_fir_idx){
                ...
                //modify 20191231 test dual echo
//                if(firings[fir_idx].distance[scan_fir_idx] > 3)
//                    std::cout<<firings[fir_idx].distance[scan_fir_idx]<<" ";
...
//lyh modified 12
void PrintPCL(std::vector<lslidar_c16_msgs::LslidarC16Point> point_cloud_ls16,double sweep_start_time = 0)
{
    static double sweep_interval = 0;
    static int sweep_cnt = 0;
    double time_interval = 0;
    double predict_interval = 200000.0/point_cloud_ls16.size();
    //    double predict_interval = 3.125;
    double err_per_pnt = 0;
    for(int i = 1;i<point_cloud_ls16.size();i++)
    {
        err_per_pnt += abs(sweep_start_time*1e6+predict_interval*i-point_cloud_ls16[i].time);
        time_interval += point_cloud_ls16[i].time - point_cloud_ls16[i-1].time;
        //        std::cout<<"time = "<<point_cloud_ls16[i].time<<std::endl;
    }
    sweep_interval += point_cloud_ls16.size() * time_interval/(point_cloud_ls16.size()-1);
    sweep_cnt++;
    std::cout<<"err_per_pnt = "<<err_per_pnt/(point_cloud_ls16.size()-1)<<std::endl;
    std::cout<<"Point Cloud Size = "<<point_cloud_ls16.size()<<" , Avg interval = "<<time_interval/(point_cloud_ls16.size()-1)<<" ,sweep interval = "<<point_cloud_ls16.size() * time_interval/(point_cloud_ls16.size()-1)<<" , pointsize*interval = "<<sweep_interval/sweep_cnt<<std::endl;
    //    PAUSE;
}
...
void LslidarC16Decoder::packetCallback(
    ...
    if (is_first_sweep &&
        ...
        if (is_first_sweep) {
            ...
            //            lyh modified 1
            double start_timeoffset = (start_fir_idx/4)*2+start_fir_idx%2;
            sweep_start_time = msg->stamp.toSec() + (FIRING_TOFFSET * (start_timeoffset)-600) * 1e-6;
            //            debuglog
            //            std::cout<<std::fixed<<std::setprecision(6)<<"sweep_start_time = "<<sweep_start_time<<" , msg->stamp.toSec()"<<msg->stamp.toSec()<<" , start_timeoffset = "<<start_timeoffset<<" , start_fir_idx = "<<start_fir_idx<<" , end_fir_idx = "<<end_fir_idx<<" , FIRING_TOFFSET = "<<FIRING_TOFFSET<<std::endl;
            //            PAUSE
        }
    }
...
//            continue;
        ...
        for (size_t scan_idx = 0; scan_idx < SCANS_PER_FIRING; ++scan_idx) {
            ...
            //                lyh modified 6
            double point_offset = (fir_idx/4)*2+fir_idx%2;
            double time = packet_start_time + FIRING_TOFFSET* point_offset + DSR_TOFFSET*scan_idx;
            time = msg->stamp.toSec()*1e6 - 600 + FIRING_TOFFSET* point_offset + DSR_TOFFSET*scan_idx;

            //            debug log
            //            std::cout<<"point_offset = "<<point_offset<<" , fir_idx = "<<fir_idx<<" , start_fir_idx = "<<start_fir_idx<<std::endl;

            // Compute the time of the point
            //            original
            //            double time = packet_start_time + FIRING_TOFFSET*fir_idx + DSR_TOFFSET*scan_idx;
            ...
            new_point.time = time - sweep_start_time*1e6;
            new_point.x = x_coord;
            new_point.y = y_coord;
            new_point.z = z_coord;
            ...
            //            lyh modified 9
            sweep_point_cloud.push_back(new_point);
        }
    }


    //        lyh modified 4
    packet_start_time += FIRING_TOFFSET*FIRINGS_PER_PACKET/2;

    //    debug log
    //    std::cout<< "packet_start_time + = "<<FIRING_TOFFSET*FIRINGS_PER_PACKET/2<<std::endl;
    //    original
    ...
    if (end_fir_idx != FIRINGS_PER_PACKET) {
        ...
        {
            //            lyh modified 12
            std::sort(sweep_point_cloud.begin(),sweep_point_cloud.end(), SortByTimestamp);

//            PrintPCL(sweep_point_cloud,sweep_start_time);
            publishPointCloud();
            sweep_point_cloud.clear();
        }
        if (publish_scan){
            publishScan();
            // publishChannelScan();
        ...
        //lyh modified 2
        double start_timeoffset = (end_fir_idx/4)*2+end_fir_idx%2;
        ...
        for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
            ...
            for (size_t scan_idx = 0; scan_idx < SCANS_PER_FIRING; ++scan_idx) {
                ...
                //                lyh modified 5
//                double point_offset = (fir_idx/4)*2+fir_idx%2 - (start_fir_idx/4)*2-start_fir_idx%2;
                double point_offset = (fir_idx/4)*2+fir_idx%2;
                double time = packet_start_time + FIRING_TOFFSET* point_offset + DSR_TOFFSET*scan_idx;
                time = msg->stamp.toSec()*1e6 - 600 + FIRING_TOFFSET* point_offset + DSR_TOFFSET*scan_idx;
                ...
                // Compute the time of the point
                //                origunal
                //                double time = packet_start_time + FIRING_TOFFSET*(fir_idx-start_fir_idx) + DSR_TOFFSET*scan_idx;
        ...
        //        lyh modified 3
        double packet_offset = (start_fir_idx/4)*2+start_fir_idx%2;
        packet_start_time += FIRING_TOFFSET*(FIRINGS_PER_PACKET/2-packet_offset);

        //        debug log
        //        std::cout<< "packet_start_time + = "<<FIRING_TOFFSET*(FIRINGS_PER_PACKET/2-packet_offset)<<std::endl;
        //        std::cout<<std::fixed<<std::setprecision(6)<<"start_fir_idx = "<<start_fir_idx<<" , end_fir_idx = "<<end_fir_idx<<" , FIRING_TOFFSET = "<<FIRING_TOFFSET<<std::endl;
